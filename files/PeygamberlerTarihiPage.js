    ...
    return (
        ...
        <ScrollView style={{ marginTop: 12, width: "100%" }}>
          {PROPHETS.map((p, index) => (
            <View key={index} style={styles.peygCard}>
              <ScaledText baseSize={14} style={styles.peygName}>{p.name}</ScaledText>
              <ScaledText baseSize={14} style={styles.peygPeriod}>{p.period}</ScaledText>
              <ScaledText baseSize={14} style={styles.peygSummary}>{p.summary}</ScaledText>
            </View>
          ))}
