...
export default function NamazPage({ onBack }) {
    ...
    return (
        ...
        <Text style={styles.guideSubtitle}>
          Farz namazlar için temel adımlar.
        </Text>

        <ScrollView style={{ marginTop: 10, width: "100%" }}>
          {sections.map((sec) => (
            <View key={sec.title} style={styles.guideCard}>
              <ScaledText baseSize={14} style={styles.guideStepTitle}> {sec.title} </ScaledText>
              <ScaledText baseSize={14} style={styles.guideStepText}> {sec.text} </ScaledText>
            </View>
          ))}
